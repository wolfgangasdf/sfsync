...
abstract class ServerView(val config: Config) extends BorderPane {
  ...
  var tfID = new MyTextField("Cache ID: ",0, "just leave it") { tf.onAction = (ae: ActionEvent) => { server.id = tf.text.value} }
  var tfFilter = new MyTextField("Filter: ",0, "e.g., (.*12)|(.*e2)") { tf.onAction = (ae: ActionEvent) => { server.filterRegexp = tf.text.value} }
